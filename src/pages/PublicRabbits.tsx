    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="pt-40 pb-20 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-black text-slate-900 tracking-tight mb-4">Available Rabbits</h1>
          <p className="text-xl text-slate-500 font-medium">Find your perfect companion or breeding stock.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-4 gap-8">
              ...
              <div className="space-y-3">
                {['New Zealand', 'Flemish Giant', 'Netherland Dwarf', 'Rex'].map(breed => (
                  <label key={breed} className="flex items-center gap-3 cursor-pointer group">
                    <div className="w-5 h-5 rounded border-2 border-emerald-100 group-hover:border-emerald-500 transition-colors" />
                    <span className="text-sm font-bold text-slate-600">{breed}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-3 grid md:grid-cols-2 xl:grid-cols-3 gap-8">
            {rabbits.map((rabbit, i) => (
              ...
              >
                <div className="aspect-square rounded-2xl bg-emerald-50 mb-6 overflow-hidden relative">
                  <img 
                    src={`https://images.unsplash.com/photo-1585110396050-c99b13f8c543?auto=format&fit=crop&q=80&w=400&sig=${i}`} 
                    alt={rabbit.name}
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                  />
                  <div className="absolute top-4 right-4 px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full text-emerald-600 font-black text-xs shadow-sm">
                    ${rabbit.price || '150'}
                  </div>
                </div>
